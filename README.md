...
第2改善フェーズを完了し、日食・月食・明るい恒星の月掩蔽を
端末内で予報する機能と、「空」「現象」に分けた主画面を実装しています。
恒星8,404件、固有名49件、
主要星座10件を同梱し、WebとmacOSで同じ計算仕様と検証値を使います。
精密モデルv2（年周視差・太陽重力光偏向・年周/日周光行差・IERS DUT1／極運動）、
順逆・速度付きの時間再生、任意の3D天球、標準／手動入力と
気象庁の最寄り観測局実測（Open-Meteoモデルfallback）からの光学大気差、
...
適用下限高度を一括検証した手動値を選べます。利用者が明示的に操作した場合だけ、
気象庁の全国最新データから端末内で最寄りの適格なアメダス観測局を選び、現在の
現地気圧・気温・相対湿度を同じ検証経路へ適用できます。実測を使えない場合だけ、
丸めた選択地点の座標をOpen-Meteoへ送りモデル値へfallbackします。
取得値と手動値はセッション中だけ保持し、
...
- [UIナビゲーション改善レビュー](docs/design/ui-navigation-review.md)
- [食・掩蔽の共有暦に関する技術判断](docs/decisions/0002-local-event-ephemeris.md)
- [現在気象を大気差へ使う技術判断](docs/decisions/0003-open-meteo-refraction.md)
...
2. 位置情報は既定で端末内だけに扱い、外部送信は目的と送信先を示した明示操作に限定する。
   手入力でも全機能を使えるようにする。
