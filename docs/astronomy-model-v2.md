...
- 見かけ恒星時
- 共有100項VSOP2000地球状態または外部の観測者位置・速度・太陽方向の準備
- 観測地点の三角関数
...
p75 5.5665 ms、p99 6.0705 ms（364 samples）だった。100項地球暦、
...
±100 Julian年での寄与上限
`abs(amplitude) × 100^timePower`が大きい100項を決定論的に選ぶ。
...
未改変SOFA `epv00`に対して1900-01-01から2100-12-31を6時間刻み
293,657時点で比較した。`natural`は地球から太陽への単位方向、
`proper`はそれへ年周光行差を適用した方向である。

| 指標 | 最大差 | RMS差 |
| --- | ---: | ---: |
| `natural`方向 | 2.2124726秒角 | 0.6127958秒角 |
| `proper`方向 | 2.2026613秒角 | 0.6129140秒角 |
| 速度ベクトル | 16.4629 m/s | 12.7934 m/s |

速度差のうち太陽方向へ効く横成分は最大0.0111462秒角であり、大部分は
省略したSSB→太陽速度である。太陽・地球距離の最大差は1,039.36 kmだった。
...
既定の観測者位置は、共有100項地球暦の太陽中心→地球位置を、太陽系重心から
...
既定proxyの主な原点差は省略したSSB→太陽位置である。さらに共有100項化の
...
- `DUT1` は利用側から渡せる。純粋な計算APIへ未指定の場合は0秒を使い
  `dut1-assumed-zero` を返す。実際の `|UT1−UTC|` は最大約0.9秒なので、
  時角には最大約13.5秒角相当の差になり得る。
...
- IERS Bulletin C 72 は2026年12月末にうるう秒を入れないことと、
  ...
  `future-leap-seconds-unknown` を返す。
...
フレーム単位contextから求める。TT時刻の共有100項地球暦で太陽中心→地球の
...
地心値のまま保持する。地点に標高がない通常UIは楕円体高0 mを明示的に
仮定し、公開APIの外部標高は日周光行差と同じWGS84地点へ適用する。

薄明は太陽中心の真空中の幾何高度を用い、境界を0°、−6°、−12°、−18°と
する。標準大気差のON/OFFは恒星の表示高度だけに作用し、薄明区分には
...
共有100項地球暦と
IAU 2000Bを通した全パイプラインの球面角残差を5秒角未満に固定する。
8ケースの実測最大は、地心赤道方向2.2032秒角、WGS84 topocenterの
水平方向2.2039秒角だった。SOFA側は`pvtob`のCIRS観測地点位置を
...
秒角と見積もる。合計0.9ミリ秒角未満であり、現行100項地球暦と歳差章動の
約2.2秒角級の残差より十分小さい。
...
既定の`e`と`em`は、年周視差・年周光行差と同じ共有100項地球暦から得る
太陽中心→地球位置を使い、
...
既定値は共有100項地球暦を解析微分した太陽中心地球速度を使う。SOFA
`epv00`が年周光行差へ使う太陽系重心地球速度とは異なり、SSB→太陽速度を
含まないため`truncated-vsop2000-heliocentric-earth`と明示する。
6時間刻み監査では速度差が最大16.4629 m/sでも、太陽方向へ効く横成分は
最大0.0111462秒角だった。位置・速度を合わせた`proper`方向の残差は、
共有100項地球暦の表に示す最大2.2026613秒角、RMS 0.6129140秒角である。
...
日周光行差、地平角、大気差の順である。既定の地点データは標高を持たないため
WGS84楕円体高0 mを明示的に仮定し、
`observer-height-assumed-zero`を返す。利用側は楕円体高を渡せ、診断用には
補正を無効化できる。無効時は`diurnal-aberration-disabled`を返し、
...
共有100項経路では地球暦方向3秒角、距離`0.00001 AU`、全パイプライン
5秒角未満を検証する。
