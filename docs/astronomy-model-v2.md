...
詳細、参考座標本文とJSONは同じ適用済み設定を使う。手動値はセッション限定で、
真空中の幾何座標は別値として常に保持する。

任意の現在気象取得では、利用者の明示操作時だけ気象庁から最新時刻、観測所表、
全国アメダスmapを取得し、選択地点の座標を送信せずに端末内で最寄り局を選ぶ。
品質フラグ0、30分以内、25 km以内で、`pressure`、`temp`、`humidity`が揃う局だけを
採用する。`pressure`は観測局の現地気圧で、海面更正気圧`normalPressure`を使わない。
観測局名、距離、平均海面基準の標高、観測時刻を画面に示すが、WGS84楕円体高との差を
暗黙に補正しない。

気象庁実測を使えない場合だけ、選択地点の緯度・経度を小数4桁へ丸めてOpen-Meteoへ送り、
`surface_pressure`、`temperature_2m`、`relative_humidity_2m`を取得する。
`pressure_msl`は現地全圧ではないため使わない。API応答のunit、有限値、範囲と時刻を
検証し、現在draftの観測波長と適用下限を保持した一つの手動設定として同じ係数検証へ
通す。失敗時は適用済み設定を変えない。JSONの`inputSource`は既存schemaの`manual`とし、
画面内だけに実測／モデル、取得元、時刻を示す。取得値は星図の任意日時に対応する
履歴気象ではない。座標・気象応答・取得元は永続保存しない。
