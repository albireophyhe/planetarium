            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    helpSection(
                        ...
                        text: "日食・月食・恒星掩蔽は同梱したJPL DE442s暦、IAU 2006/2000B、WGS84、利用可能な範囲ではIERS DUT1・極運動を用いて計算します。月縁は平均半径で、月の山谷によるベイリー・ビーズや掩蔽の瞬間的な明滅は再現しません。恒星掩蔽はBSC5Pの明るい恒星を対象にした参考予報で、潜入・最接近・出現、対象星の高度・方位、月縁位置角を表示します。平均月縁の物理境界帯内では掩蔽の発生を確定せず、候補として最接近時刻だけを示します。地点の標高は都市・手入力では0 m、端末測位ではOSの値を使います。水平精度も端末測位ではOSの値を境界幅へ加え、都市・手入力では不明として扱います。大気差、地形、建物、雲、視程は含みません。各予報のInspectorに前提、不確かさ、出典を表示します。太陽を肉眼や光学機器で直接見ないでください。皆既日食でも、皆既中以外は規格に適合した日食観察用フィルターが必要です。"
                    )
                    ...
                    helpSection(
                        ...
                        text: "正の星表視差値がある星だけ、地球の公転に伴う見かけの方向変化を計算します。既定の観測者位置には、共有するVSOP2000由来200項の太陽中心地球暦をproxyとして使います。この暦はTTをTDBのproxyとして評価し、厳密な太陽系重心から太陽までの変位と、地球中心から実観測地点までの日周視差は含みません。視線速度が未収録でも距離を捨てず、0 km/sを仮定して年周視差を適用します。その場合は未知の遠近加速度を含みません。視差値がない星は「変位なし」ではなく「計算不可」です。いずれも精密観測や望遠鏡導入用の補正ではありません。"
                    )
