            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    helpSection(
                        ...
                        text: "UT1−UTC（DUT1）と極運動xp・ypには、2026年7月29日取得のIERS Bulletin A finals2000Aスナップショットを使います。収録範囲は1973-01-02〜2027-07-31 UTCで、2026-07-23までは観測値、翌日以降は将来の更新で変わり得る予測値です。DUT1は00:00 UTCの日次値を線形補間し、うるう秒による約1秒の段差を前日へsmearせず、翌日00:00で表値へ切り替えます。xp・ypは公式SOFA例と同じ4点Lagrange補間です。DUT1と極運動の観測／予測区分・公表誤差は独立して扱います。公式formatは誤差列を単にerrorと定義しており、1σとは仮定しません。範囲外またはデータを検証して読み込めない場合はDUT1=0秒、xp=yp=0の近似へ戻り、画面にその状態を表示します。時角の最大約13.5秒角は、現行の整数うるう秒UTCを前提にしたDUT1だけの条件付き目安です。xp/yp=0による方向差も同梱履歴では最大約0.6秒角です。1972年以前はTAI−UTC=0秒、将来は既知最後の37秒を仮定するUTC近似を含みます。読込・検証・適用に失敗した場合はInspectorの「IERSデータを再読み込み」で再試行できます。"
                    )
