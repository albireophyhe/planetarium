            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    helpSection(
                        title: "精密モデルv2と大気差",
                        systemImage: "scope",
                        text: "既定は大気差OFFで、真空中の幾何高度を表示します。ONへ切り替えると、気圧1013.25 hPa・気温10°C・相対湿度50%・観測波長0.55 µmを仮定した標準大気を幾何高度5°以上へ適用します。「大気差を詳しく設定」では、このセッションだけの気圧・気温・相対湿度・光学から近赤外（0.3〜2 µm）の観測波長と5〜30°の最低適用高度を手動入力できます。「現在の気象を取得して適用」を押した場合だけ、気象庁の全国最新データから端末内で最寄りの適格な観測局を選び、現地気圧・気温・相対湿度の実測を同じ検証経路へまとめて反映します。気象庁には選択地点の座標を送りません。30分以内かつ25 km以内の良好な3値が揃わない場合だけ、丸めた座標をOpen-Meteoへ送り15分間隔の気象モデル値へ切り替えます。画面には実測／モデル、観測局名、距離、標高、時刻を区別して表示します。どちらも星図の日時とは自動同期しません。最低適用高度より低い空は大気の鉛直構造に敏感なため補正しません。手動入力と取得値でも天候の時間変化や鉛直構造を再現するものではありません。恒星位置にはBSC5P J2000.0 FK5座標をSOFA由来のJ2000回転・フレームスピンでHipparcos/ICRSへ接続し、固有運動、近似年周視差、太陽重力光偏向、近似年周光行差、IAU 2006 Fukushima–Williams歳差、IAU 2000B 77項章動、IERS DUT1を反映した見かけの恒星時、IERS極運動、WGS84日周光行差の順で適用します。極運動はGASTからTIRSへ回した後、SOFA由来のTIO locator s′とpom00相当の行列でITRSへ変換して地点のENU座標へ移します。日内の極運動潮汐、惑星による光偏向、恒星の日周視差は未適用です。太陽の水平位置だけはWGS84地点変位による日周視差を含みます。IERS収録期間内の「概ね1〜数秒角級」は、BSC5Pの格納分解能から見た真空中の通常目安で、全恒星の実測精度を保証する値ではありません。大気差ON時の表示高度は別です。BSC5PのFK5ゾーン誤差、共有地球暦の200項への切り詰め、TTをTDBのproxyとすること、太陽系重心に対する太陽速度の省略も精度を制限するため、サブ秒角精度は主張しません。この説明は通常の恒星星図に関するもので、食予報は別のDE442s計算経路を使います。詳細画面の座標転記は、座標系と大気差設定を確認した手動導入・機材設定比較の実験的な補助情報です。無人の自動導入・追尾、掩蔽の成立判定、測地、航法の唯一の入力には使用しないでください。"
                    )

                    helpSection(
                        title: "プライバシー",
                        systemImage: "hand.raised",
                        text: "起動時に位置権限を求めません。「現在地」を押した場合だけmacOSへ許可を要求します。正確な座標、日時、検索語、選択した星は保存しません。現在気象の明示ボタンを押した場合だけ気象データを取得します。気象庁には座標を送らず、適格な実測を使えない場合だけ小数4桁へ丸めた緯度・経度をOpen-Meteoへ送信します。日時、地点名、検索語、選択した星、測位精度と高さは送りません。取得した気象値も保存せず、次回起動時に大気差がONなら標準大気へ戻します。端末へ保存するのは、星座線、星の名前、選択星の軌跡、ナイトモード、大気差ON/OFFの5表示設定だけで、設定画面から消去できます。星表と計算は端末内に同梱され、気象取得なしでも利用できます。Open-Meteo無料APIは非商用向けで、データはCC BY 4.0です。"
                    )
                    ...
                    VStack(alignment: .leading, spacing: 7) {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Link(
                                "気象庁公開データを加工して利用",
                                destination: URL(string: "https://www.jma.go.jp/bosai/amedas/")!
                            )
                            Link(
                                "Weather data by Open-Meteo.com",
                                destination: URL(string: "https://open-meteo.com/en/licence")!
                            )
                        }
                    }
